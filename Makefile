...
################################################
# Remote SSH programming

SSH_TARGET      ?= root@192.168.0.10
REMOTE_RBF_TARGET  ?= ~/tmp/soc_system.rbf

.PHONY: prog_ssh
prog_ssh: $(QUARTUS_RBF)
	@echo "→ copying $(QUARTUS_RBF) to $(SSH_TARGET):$(REMOTE_RBF_TARGET)"
	scp $(QUARTUS_RBF) $(SSH_TARGET):$(REMOTE_RBF_TARGET)
	@echo "→ rebooting $(SSH_TARGET)"
	ssh $(SSH_TARGET) mount 
	
################################################


################################################
# Archive to .qar and stage the exact source files into Git

.PHONY: qar_add
HELP_TARGETS += qar_add
qar_add.HELP := Archive to QAR and git-add all files that went into it

qar_add:
	@echo "→ Creating Quartus .qar archive: $(QUARTUS_BASE).qar"
	@quartus_sh --archive $(QUARTUS_BASE) -use_file_set basic -output $(QUARTUS_BASE).qar
	@echo "→ Parsing $(QUARTUS_BASE).qarlog for files to stage…"
	@sed -n '/Files Selected:/,/Total:/p' $(QUARTUS_BASE).qarlog \
		| sed '1d;$$d' \
